...
export const Card = ({ transaction, removeFromListTransaction }) => {
  return (
    <>
      {transaction.type == "Entrada" ? (
        <li style={{ borderLeft: "5px solid #03B898" }}>
          <h2 className="description">{transaction.description} </h2>
          <p className="type">{transaction.type} </p>
          <p className="value"> R$ {transaction.value},00 </p>
          <button
            className="remove"
            onClick={() => removeFromListTransaction(transaction.id)}
          >
            <svg
              width="9"
              height="10"
